...
public class Canvas { // todo - delete this
	...
	public void drawText(String s, float x, float y) {
		//todo g.setFont(Statics.stdfnt); // Default
		/*if (p != null) {
			g.setColor(p.color);
	...
	public void drawCircleFromCentre(float x1, float y1, float r) {
		/*if (p != null) {
			g.setColor(p.color);
			Graphics2D g2 = (Graphics2D)g;
			g2.setStroke(new BasicStroke(p.getStrokeWidth()));
	...
	public void drawRect(RectF r) {
		drawRect(r.left, r.top, r.right, r.bottom);
	}


	public void drawBitmap(BufferedImage b, float x, float y) {
		g.drawImage(b, (int)x, (int)y, null);
	}


	public void drawImage(BufferedImage b, float x, float y) {
		g.drawImage(b, (int)x, (int)y, null);
	}
